# O Grande Código da Gralha Azul

**The Great Code of the Azure Jay** — CRSF/PPM servo ornithopter library for RP2040.

---

## Quick Start

```cpp
#include <GralhaAzul.h>

void setup()  { gralhaAzulSetup(); }
void loop()   { gralhaAzulLoop();  }
```

Open `o-grande-codigo-da-gralha-azul.ino` in Arduino IDE — it automatically includes all library files.

---

## Wiring

### Minimal (CRSF + 2 servos)

```
RP2040 Zero              Servo Left (GP5)    Servo Right (GP7)
┌─────────────┐          ┌──────────┐        ┌──────────┐
│ GPIO 5  [~]─►──────────│ Signal   │        │ Signal   │
│ GPIO 7  [~]─►──────────│          │════════│          │
│ GPIO 0 TX ───► (yel) ─► ELRS RX  │        │          │
│ GPIO 1 RX ◄─── (blu) ◄── ELRS TX │        │          │
│ VBUS ──────────► ELRS VCC        │        │          │
│ GND  ◄─── GND ◄──── ELRS GND ◄──┼────────┼──────────┤
└─────────────┘                    │ red ───┼── 5V BEC │
                                   │ blk ───┼── GND    │
                                   └────────┘          │
                                                        │
External 5V BEC ──► 5V ────────────────────────────────┘
```

- **Common ground** between RP2040, servos, and ELRS receiver
- **Do not power servos from RP2040 3.3V** — use external BEC
- CRSF baud: 420000 (`FREQUENCIA_DO_SOPRO_COSMICO`)

### Pin Allocation

| Pin    | Function                | Portuguese Name                |
|--------|-------------------------|--------------------------------|
| GPIO 5 | Servo — Left Wing       | `ARTICULACAO_ASA_DA_MANHA`     |
| GPIO 7 | Servo — Right Wing      | `ARTICULACAO_ASA_DO_ENTARDECER`|
| GPIO 0 | CRSF TX                 | `VIA_DOS_ECOS_SOLARES`         |
| GPIO 1 | CRSF RX                 | `VIA_DOS_SONHOS_LUNARES`       |
| GPIO 16| NeoPixel (onboard)      | `NUCLEO_DA_CHAMA_AZUL`         |

PPM alternative: set pin via `PINO_DO_MENSAGEIRO` (default GPIO 2).

---

## Configuration

The single configuration file: `src/GralhaAzulConfig.h`

**Key parameters to tune per model:**

| Define                  | Default  | Description                                |
|-------------------------|----------|--------------------------------------------|
| `CICLO_DO_CORACAO_ALADO` | `0.052f` | Wingbeat period (seconds). See servo table |
| `ANGULO_DO_PLANAR_SERENO`| `-4`     | Glide angle (degrees, negative = descend)  |
| `ORIGEM_ASA_MATUTINA`   | `0`      | Left wing neutral offset                   |
| `ORIGEM_ASA_VESPERTINA` | `0`      | Right wing neutral offset                  |
| `FEROCIDADE_MINIMA`     | `1.0f`   | Minimum stroke amplitude                   |
| `FEROCIDADE_MAXIMA`     | `8.0f`   | Maximum stroke amplitude                   |

### Optional Modules (disable before `#include`)

```cpp
#define GRALHA_AZUL_NEOPIXEL_DESLIGADO
#define GRALHA_AZUL_BAROMETRO_DESLIGADO
#define GRALHA_AZUL_TELEMETRIA_DESLIGADO
```

> **Compilation note**: Since GralhaAzul is a compiled library, `#define`s in your `.ino` are not automatically visible to the library `.cpp`. Either edit `GralhaAzulConfig.h` directly (recommended) or pass `-D` flags via PlatformIO build config.

---

## Servo Recommendations

| Example              | Servo                     | Speed (60°) | Torque    | `CICLO_DO_CORACAO_ALADO` | Rate   |
|----------------------|---------------------------|-------------|-----------|---------------------------|--------|
| `CRSF_Exemplo`       | DSSERVO DS3218 MG (25g)   | 0.17s @6V   | 3 kg·cm   | `0.052f`                 | ~19 Hz |
| `PPM_Exemplo`        | DSSERVO DS3218 MG (25g)   | 0.17s @6V   | 3 kg·cm   | `0.052f`                 | ~19 Hz |
| `CRSF_Servo_Rapido`  | Blue Arrow D0576HT (1.7g) | 0.07s @5V   | 0.2 kg·cm | `0.030f`                 | ~33 Hz |
| `CRSF_Servo_Torque`  | KST MS320 (56g)           | 0.14s @8.4V | 8.5 kg·cm | `0.080f`                 | ~12 Hz |

To calculate your own `CICLO_DO_CORACAO_ALADO`: take the servo's rated speed for 60° movement at your supply voltage, add ~20% safety margin, and use that as the minimum period. Slower = safer, faster = risk of overheating.

---

## CRSF Channel Mapping

| Chan | Source                   | Function              |
|------|--------------------------|-----------------------|
| CH1  | Aileron (right stick H)  | Roll / differential   |
| CH2  | Elevator (right stick V) | Pitch                 |
| CH3  | Throttle (left stick V)  | Wingbeat intensity    |
| CH4  | Rudder (left stick H)    | Yaw / steering        |
| CH5  | (unused)                 | Arm switch            |
| CH6  | Potentiometer            | Rhythm                |
| CH7  | Potentiometer            | Downstroke ferocity   |
| CH8  | Potentiometer            | Upstroke ferocity     |
| CH9  | Potentiometer            | Rudder differential   |
| CH10 | Potentiometer (gain)     | Altitude hold gain    |

PPM: CH1–CH8 only (CH9+10 require `#define`).

---

## Receiver Setup

### ExpressLRS / CRSF

| Parameter | Setting  | Reason                        |
|-----------|----------|-------------------------------|
| Protocol  | `CRSF`   | Serial output                 |
| Baud      | `420000` | Matches firmware default      |
| Rate      | ≥50 Hz   | Good for ornithopter          |

### PPM

```cpp
#define RECEPTOR_DOS_VENTOS_PPM
```

PPM input on GPIO2, up to 8 channels.

---

## FAQ

**Q: Servo gets hot mid-flight.**  
A: Reduce wingbeat rate — increase `CICLO_DO_CORACAO_ALADO`. Check servo speed rating.

**Q: No response from receiver.**  
A: Verify CRSF wiring (TX↔RX crossover). Check baud rate (420000). Confirm receiver flashed with CRSF protocol.

**Q: Barometer not detected.**  
A: Check I2C wiring (GP4=SDA, GP5=SCL). Ensure BMP180 is 3.3V compatible.

**Q: NeoPixel stays dark.**  
A: Confirm `GRALHA_AZUL_NEOPIXEL_DESLIGADO` is not defined. Check GPIO 16 connection.

---

## License

MIT — share, build, fly.
